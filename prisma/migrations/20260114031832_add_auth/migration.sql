/*
  ...
  - A unique constraint covering the columns `[userId]` on the table `AppSetting` will be added. If there are existing duplicate values, this will fail.
  - A unique constraint covering the columns `[userId]` on the table `ChatPreference` will be added. If there are existing duplicate values, this will fail.
  ...
  - Added the required column `userId` to the `ChatPreference` table without a default value. This is not possible if the table is not empty.
  ...
  - Added the required column `userId` to the `KnowledgeText` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE "ChatPreference" ADD COLUMN     "userId" TEXT NOT NULL;

-- AlterTable
ALTER TABLE "KnowledgeFile" ADD COLUMN     "userId" TEXT NOT NULL;

-- AlterTable
ALTER TABLE "KnowledgeText" ADD COLUMN     "userId" TEXT NOT NULL;
...
-- AddForeignKey
ALTER TABLE "KnowledgeText" ADD CONSTRAINT "KnowledgeText_userId_fkey" FOREIGN KEY ("userId") REFERENCES "User"("id") ON DELETE CASCADE ON UPDATE CASCADE;
